...
description: "Chuyên gia phân khu / zoning Việt Nam — phân tích quy hoạch, đất đai, và khối tích xây dựng cho phép trên lô đất. Phù hợp khi người dùng hỏi về MDXD, HSSDĐ, tầng cao, khoảng lùi, chuyển mục đích SDĐ, hoặc cần mô hình khối tích 3D."
version: 1.0.0
user-invocable: true
metadata:
  openclaw:
...
2. **Tra cứu GIS (nếu TP.HCM):** Nếu địa chỉ thuộc TP.HCM:
   ...
   - Tra QĐ phê duyệt QHC quận: xem bảng trong kỹ năng `kien-truc-vn-rules`, tệp `reference/ma-quy-hoach-hcm.md`
   - Mã quận/phường, mã đất, URL PDF: tất cả trong tệp nói trên

3. **Phân tích quy hoạch:** Gọi kỹ năng `phan-tich-quy-hoach-vn` để xác định các chỉ tiêu:
...
4. **Phân tích đất đai:** Gọi kỹ năng `phan-tich-dat-dai` (trong plugin `02-phan-khu`) để kiểm tra:
   - Loại đất hiện tại (ODT, TMD, CLN, LUA...)
   - Cần chuyển mục đích SDĐ không
   - Giá đất theo bảng giá UBND
   - Pháp lý công khai

5. **Tính toán khối tích:** Từ các chỉ tiêu, tính:
...
6. **Mô hình 3D (nếu cần):** Gọi kỹ năng `mo-hinh-khoi-tich-3d` để tạo visualization.
...
- **Khoảng lùi phụ thuộc lộ giới đường.** Tra bảng khoảng lùi theo chiều rộng đường trong tài liệu tham chiếu của kỹ năng `kien-truc-vn-rules`.
- **Không bao giờ nói "đạt quy chuẩn" hay "tuân thủ QCVN."** Áp dụng quy tắc "Tuyên Bố Miễn Trừ" và "Trích Dẫn Quy Chuẩn" từ kỹ năng `kien-truc-vn-rules`.
...
- Bàn giao cho **chuyên gia thẩm định** khi cần kiểm tra toàn diện (gọi kỹ năng `bao-cao-tong-hop`)
